...
public class LevelManager : MonoBehaviour
{
    ...
    /*void SetupSecondSetOfQuestions()
    {
        // Activate the buttons
        choiceButton1.gameObject.SetActive(true);
        choiceButton2.gameObject.SetActive(true);

        // Set button text (assuming each button has a child TextMeshPro component)
        choiceButton1.GetComponentInChildren<TextMeshProUGUI>().text = "Option 1: Correct";
        choiceButton2.GetComponentInChildren<TextMeshProUGUI>().text = "Option 2: Incorrect";

        // Add listeners to buttons
        choiceButton1.onClick.AddListener(() => CheckButtonAnswer(true));
        choiceButton2.onClick.AddListener(() => CheckButtonAnswer(false));
    }*/

    void CheckButtonAnswer(bool isCorrect)
    {
        if (isCorrect)
        {
            Debug.Log("Correct answer!");
            // Optionally, proceed to another question or end this set
        }
        else
        {
